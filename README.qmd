...
As shown in the graph above, some records (`r round(nrow(d_offence) / nrow(d_all) * 100, 1)`%) have missing values for the 'offence' column (these records did not classify as an offence). 
Many records lack either an offence or a location (`r round(nrow(d) / nrow(d_all) * 100, 1)`%).
Filtering these out leaves `r nrow(d)` complete records.
